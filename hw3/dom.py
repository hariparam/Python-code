...
class DominoesGame(object):
    ...
    def get_max(self, node, vertical, alpha, beta, depth, limit, nodes_visited):
        if depth == limit:
            ...
            return (node, -opponentmoves , nodes_visited)#len(list(newboard.legal_moves(not vertical)))
        v = -1000
    ...
    def get_min(self,node,vertical, alpha, beta, depth, limit,nodes_visited):
        if depth == limit:
            nodes_visited += 1
            newboard = self.copy()
            newboard.perform_move(node[0], node[1], vertical)
            playermoves = len(list(newboard.legal_moves(vertical)))
            opponentmoves = len(list(newboard.legal_moves(not vertical)))
            return (node, len(list(newboard.legal_moves(not vertical))), nodes_visited)  # len(list(newboard.legal_moves(not vertical)))
        v = 1000
        for m, new_g in self.successors(vertical):
